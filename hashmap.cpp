...
int hashmap::get(int key) const{
  ...
  key_val_pair* temp = buckets[index];
...
void hashmap::put(int key, int value) {
  ...
  // TODO: Implement this function.
  //
  // check if element already in hashmap
  if (containsKey(key)){
    int index = hashFunction(key) % nBuckets;

    key_val_pair* temp = buckets[index];; // do nothing 
    while (temp != nullptr){
      if (temp->key == key){
        temp->value = value;
      }
  ...
  // if not in hashMap then get Hashed index
  int index = hashFunction(key) % nBuckets;
  key_val_pair* toInsert = new key_val_pair();
  toInsert->key = key;
  ...
  if (buckets[index] == nullptr){
    buckets[index] = toInsert;
    // cout << buckets[index]->key << endl;
  } else {
    while (temp->next != nullptr){
      // cout << buckets[index]->value;
      temp = temp->next;
    }
    temp->next = toInsert;
  }
  
  //cout << buckets[index]->value << endl;
  nElems++;
}
...
// int hashmap::printBucket(){
//   key_val_pair* temp = buckets[9];
//   while (temp != nullptr){
